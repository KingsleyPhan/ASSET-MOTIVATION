...
@Controller
@RequestMapping("KTTaiSanLenhDiChuyenNoiBo")
public class KTTaiSanLenhDiChuyenNoiBoContorller {
	...
	public ModelAndView init(HttpServletRequest request, Model model) {
		...
		if(UtilCommon.isEmpty(CouponCd)) {
			...
			try {
				...
				mv.setViewName("KTTaiSanLenhDiChuyenNoiBo");
	...
	public ModelAndView approve(HttpServletRequest request, Model model, @ModelAttribute("TaoLenhDiChuyenNoiBoForm") TaoLenhDiChuyenNoiBoForm form, BindingResult result) {
		...
		lenh.setCommentAccount(form.getCommentAccount());
		lenh.setApproveAccountDt(UtilCommon.getDateCurrent("dd/MM/yyyy"));
		lenh.setApproveAccountUser((String) request.getSession().getAttribute(SessionParams.SESSION_USER_NAME));
