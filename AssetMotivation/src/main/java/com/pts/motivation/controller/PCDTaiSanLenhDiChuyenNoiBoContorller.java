...
@Controller
@RequestMapping("PCDTaiSanLenhDiChuyenNoiBo")
public class PCDTaiSanLenhDiChuyenNoiBoContorller {
	...
	public ModelAndView init(HttpServletRequest request, Model model) {
		...
		if(UtilCommon.isEmpty(CouponCd)) {
			...
			try {
				...
				mv.setViewName("PCDTaiSanLenhDiChuyenNoiBo");
	...
	public ModelAndView approve(HttpServletRequest request, Model model, @ModelAttribute("TaoLenhDiChuyenNoiBoForm") TaoLenhDiChuyenNoiBoForm form, BindingResult result) {
		...
		lenh.setCommentPCD(form.getCommentPCD());
		lenh.setApprovePCDDt(UtilCommon.getDateCurrent("dd/MM/yyyy"));
		lenh.setApprovePCDUser((String) request.getSession().getAttribute(SessionParams.SESSION_USER_NAME));
